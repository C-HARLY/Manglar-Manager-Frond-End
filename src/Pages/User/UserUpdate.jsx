...
export const UserUpdate = () => {
  ...
  const getSingleUser = async () => {
    try {
      const { data } = await axios.get(`http://localhost:3005/user/getOne/${id}`, { headers: headers })
      if (data.findUser) {
  ...
  const updateUser = async () => {
    try {
      ...
      const { data } = await axios.put(`http://localhost:3005/user/update/${id}`, upUser, { headers: headers })
      if (data.message) {
