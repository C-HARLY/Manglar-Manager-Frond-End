...
export const TaskPage = () => {
  ...
  const getDepartments = async () => {
    try {
      const { data } = await axios.get('http://localhost:3005/dep/getDeps',)
      if (data.department) {
  ...
  const getTask = async () => {
    try {
      const { data } = await axios.get('http://localhost:3005/task/get', { headers: headers })
      if (data.gettasks) {
  ...
  const getUsers = async () => {
    try {
      const { data } = await axios.get('http://localhost:3005/user/get', { headers: headers })
  ...
  const addTask = async () => {
    try {
      ...
      const { data } = await axios.post(`http://localhost:3005/task/assign`, newTask, { headers: headers })
      if (data) {
        getTask();
        Swal.fire({
          icon: 'success',
