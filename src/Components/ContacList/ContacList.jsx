  ...
  export const ContactListTable = () => {
    ...
    const currentUserId = 'ID_DEL_USUARIO_LOGUEADO';
    ...
    const fetchUsers = async () => {
      try {
        const response = await axios.get('http://localhost:3005/user/todos', { headers: headers });
    ...
    const handleSendMessage = async () => {
      try {
        ...
        const response = await axios.post(`http://localhost:3005/chat/sendmessage `,  {
          ...
          <tbody>
            {filteredUsers.map((user) => (
              <tr key={user._id}>
                ...
                <td>
                  <span>{user.departament ? user.departament.name : 'N/A'}</span>
                </td>
                <td>
                  <MDBBtn
                    className="btn"
