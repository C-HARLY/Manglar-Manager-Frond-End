...
export const ProfilePage = () => {
  ...
  const getProfile = async () => {
    try {
      const { data } = await axios.get('http://localhost:3005/user/getProfile', { headers: headers })
      if (data.findToken) {
  ...
  const getDeparts = async () => {
    try {
      const { data } = await axios.get(`http://localhost:3005/dep/getDeps`, { headers: headers })
      if (data.department) {
  ...
  const addComplaiment = async () => {
    try {
      ...
      const { data } = await axios.post('http://localhost:3005/comp/addComp', Addcomp, { headers: headers })
      if (data) {
  ...
  const getYourTasks = async () => {
    try {
      const { data } = await axios.get('http://localhost:3005/task/LoginTask', { headers: headers })
      if (data.findTask) {
